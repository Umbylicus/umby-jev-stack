...
description: >-
  ...
  findings before fixes. Independent Noul questions per hunt (syntax, secrets,
  injection/XSS, auth, schema, logic). Frozen compile labels: Clean_Codebase,
  Syntax_Or_Type_Error, Vulnerability_Flagged, Schema_Mismatch. Never pick one winner;
  never drop a finding; fix only after user reviews. Requires JEV_API_KEY.
license: MIT
metadata:
  author: umby
  version: "1.1.0"
